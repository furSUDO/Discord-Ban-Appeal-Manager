                            ...
                            if (result[0].appealServer === message.guild.id) {
                                ...
                                message.guild.members.fetch(userID, {force: true} ).then(async guildMember => {
                                    ...
                                    parentGuild.fetchInvites().then(async invites => {
                                        guildMember.send(`Your ban appeal on ${parentGuild.name} has been soft denied!\nAs a result, You can join the appeal server again if you wish to reappeal!`).then(guildMember.kick())
                                        .catch(error => {
                                    ...
                                    .then(async messages =>{
                                        ...
                                        message.reply({embeds: [{color: 15088700,title:`<:warning:847479424583729213> Failed to log event`,description:`Please create a channel with the name \`logs\` or run \`!init\` to format the server properly.\n\nThis channel will not delete automatically.`,fields:[{
                                            ...
                                            logChannel.send({
                                                embeds: [{
                                                    color: 16763724,
                                                    title:`⚠ ${guildMember.user.username} (${guildMember.user.id}) has been soft denied, this means they can join back and reappeal!`,
                                                    fields:[{
                                    ...
                                    .then(async messages =>{
                                        ...
                                        message.reply({embeds: [{color: 15088700,title:`<:warning:847479424583729213> Failed to log event`,description:`Please create a channel with the name \`logs\` or run \`!init\` to format the server properly.\n\nThis channel will not delete automatically.`,fields:[{
                                            ...
                                            logChannel.send({
                                                embeds: [{
                                                    color: 16763724,
                                                    title:`⚠ ${guildMember.username} (${guildMember.id}) has been soft denied, this means they can join back and reappeal!`,
                                                    fields:[{
                                                        name:`Responsible Moderator`,
                                                        value:`${message.author.username}`
                                                    },{
